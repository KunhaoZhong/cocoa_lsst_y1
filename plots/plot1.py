...
#DELETE TMP FILES
subprocess.Popen("rm KZ_P1_TMP[0-9].*", shell=True, cwd=".")
...
parameter = [u'omegam', u'sigma8',u'chi2']
chaindir=r'.'

root_chains = ('EXAMPLE_MCMC_SPK_4', 'EXAMPLE_MCMC_SPK_3')
...
# --------------------------------------------------------------------------------
# --------------------------------------------------------------------------------
samples=loadMCSamples('../chains/' + root_chains[0],settings=analysissettings)
p = samples.getParams()
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
samples.saveAsText('KZ_P1_TMP1')
# --------------------------------------------------------------------------------
# --------------------------------------------------------------------------------
# --------------------------------------------------------------------------------
samples=loadMCSamples('../chains/' + root_chains[1],settings=analysissettings)
p = samples.getParams()
# --------------------------------------------------------------------------------
# samples.addDerived(np.exp(p.logA)/10.0,name='1e9As',label='{10^9\\rm A_s}',
# range=[np.exp(samples.getLower('logA'))/10.0,np.exp(samples.getUpper('logA'))/10.0])
...
param_3d = None
g.triangle_plot(['KZ_P1_TMP1','KZ_P1_TMP2'],parameter,
...
legend_labels=['No Baryon marginalization',
               'Baryon with Spk emulator',],
                
legend_loc=(0.39,0.795),
imax_shaded=0)
g.export()
